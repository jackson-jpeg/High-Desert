...
| A8 | **On a phone the Studio opened scrolled down to the log's ON AIR row.** The clock, the live count and Now playing were above the top of the screen. The guide called `scrollIntoView`, which scrolls every scrolling ancestor, and on a phone the log's box grows to fit its rows, so the page scrolled. It only shows when the show on air is far down the day's log, which is why it first went red on CI at 6 PM Pacific (after the 16a8f49 deploy) and was then reproduced on production. | `revealInGuideBox()` scrolls the log's own box, and only when that box overflows (desktop). | `program-guide-scroll.test.tsx`; e2e "nothing in the studio is wider than the screen" (its in-viewport check); mutations `live-guide-never-scrolls-page`, `live-guide-reveals-on-air` |
