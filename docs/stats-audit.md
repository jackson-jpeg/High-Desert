...
14. **The listening milestone counted position, not time heard. Fixed 2026-09-26.**
    `MilestoneDialog` summed `progress.playbackPosition`, the same mistake finding-era
    "Listened" made. On the live station that is where the show is, so a first-time visitor
