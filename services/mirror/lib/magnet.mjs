/**
 * A magnet URI for an episode's torrent: infohash, name, the trackers, the
 * archive.org webseed (`ws=`, so a client can fetch while archive.org is up
 * even with no peers), and this server as a peer hint (`x.pe=`, BEP 9) — the
 * gateway seeds the pinned shows back.
 */
