...
export const MUTATIONS = [
  ...
  },
  {
    ...
    test: "src/lib/__tests__/service-worker.test.ts",
    file: "public/sw.js",
    find: "  self.skipWaiting();\n});",
    replace: "});",
    why: "without skipWaiting a deploy's worker waits until every tab closes, leaving returning visitors on the old build",
  },
  {
    id: "sw-clients-claim",
    test: "src/lib/__tests__/service-worker.test.ts",
    file: "public/sw.js",
    find: "      .then(() => self.clients.claim())",
    replace: "      .then(() => undefined)",
    why: "without claim the new worker does not take over the pages already open",
  },
  {
    id: "sw-activate-purges-old-build",
    test: "src/lib/__tests__/service-worker.test.ts",
    file: "public/sw.js",
    find: "keys.filter((k) => k !== CACHE_NAME).map",
    replace: "keys.filter(() => false).map",
    why: "an old build's shell left in cache can be served offline as if it were current",
  },
  {
    id: "sw-navigation-network-first",
    test: "src/lib/__tests__/service-worker.test.ts",
    file: "public/sw.js",
    find: "      fetch(event.request)\n        .then((response) => {\n          // Never cache errors",
    replace: "      caches.match(event.request).then((c) => c || fetch(event.request))\n        .then((response) => {\n          // Never cache errors",
  ...
  },
  {
    ...
    file: "src/hooks/useShellMenus.ts",
    find: `        { label: "Surprise Me: Shuffle All", onClick: () => openLibrary({ shuffle: "all" }) },`,
    replace: `        { label: "Surprise Me \u2014 Shuffle All", onClick: () => openLibrary({ shuffle: "all" }) },`,
  ...
  },
  {
    ...
    test: "src/lib/__tests__/no-em-dash.test.ts",
    file: "services/live/lib/app.mjs",
    find: `message: "New callers from your network are on hold for a while. Try again later." },`,
    replace: `message: "New callers from your network are on hold for a while \u2014 try again later." },`,
    why: "the phone lines' refusals are copy callers read; a scan of src/ alone would miss the service",
  },
  {
    id: "no-em-dash-manifest",
    test: "src/lib/__tests__/no-em-dash.test.ts",
    file: "public/manifest.json",
    find: `  "name": "High Desert · Art Bell Radio Archive",`,
    replace: `  "name": "High Desert \u2014 Art Bell Radio Archive",`,
    why: "the installed app's name is copy too, and it is JSON, not source",
  },
  {
  ...
  {
    id: "live-caller-per-browser",
    test: "services/live/test/callers.db.test.mjs",
    file: "services/live/lib/app.mjs",
    find: "    const refFor = (id) => callerRef(id, clientSecret);",
    replace: "    const refFor = () => addr;",
    needs: "TEST_DATABASE_URL",
    why: "two browsers behind one address (a household, a carrier NAT) are two callers, with their own names, lines and rename limits",
  },
  {
    id: "live-caller-line-per-address",
    test: "services/live/test/callers.db.test.mjs",
    file: "services/live/lib/store.mjs",
    find: "          if (!used.has(candidate)) {",
    replace: "          if (true) {",
    needs: "TEST_DATABASE_URL",
    why: "two browsers in one household are on two different lines",
  },
  {
    id: "live-caller-cookie-signed",
    test: "services/live/test/callers.db.test.mjs",
    file: "services/live/lib/caller.mjs",
    find: "  return want.length === got.length && timingSafeEqual(want, got) ? m[1] : null;",
    replace: "  return m[1];",
    needs: "TEST_DATABASE_URL",
    why: "a caller id the service did not mint is refused, or anyone could invent fresh ids past the address limits",
  },
  {
    id: "live-mute-per-caller",
    test: "services/live/test/callers.db.test.mjs",
    file: "services/live/lib/store.mjs",
    find: "        `SELECT until, reason FROM live_mutes WHERE client_ref = $1 AND until > now()`,",
    replace: "        `SELECT until, reason FROM live_mutes WHERE until > now() AND client_ref IN (SELECT client_ref FROM live_names WHERE addr_ref = (SELECT addr_ref FROM live_names WHERE client_ref = $1))`,",
    needs: "TEST_DATABASE_URL",
    why: "muting one caller must not mute everyone behind their address",
  },
  {
    id: "live-ban-holds-address",
    test: "services/live/test/callers.db.test.mjs",
    file: "services/live/lib/app.mjs",
    find: "        await store.holdAddress(m.addrRef, now() + BAN_HOLD_MS, now() + BAN_HOLD_INTERVAL_MS);\n",
    replace: "",
    needs: "TEST_DATABASE_URL",
    why: "a ban holds the address, so clearing cookies is not a free reset",
  },
  {
    id: "live-address-hold-enforced",
    test: "services/live/test/callers.db.test.mjs",
    file: "services/live/lib/app.mjs",
    find: "    if (!held.ok) {",
    replace: "    if (false) {",
    needs: "TEST_DATABASE_URL",
    why: "a new caller from a held address waits out the hold's interval",
  },
  {
    id: "live-first-call-cap",
    test: "services/live/test/callers.db.test.mjs",
    file: "services/live/lib/app.mjs",
    find: "    const t = firstCallCounter.take(addr);\n    if (!t.ok) {",
    replace: "    const t = firstCallCounter.take(addr);\n    if (false) {",
    needs: "TEST_DATABASE_URL",
    why: "new callers starting to talk are capped per address",
  },
  {
    id: "live-new-caller-cap",
    test: "services/live/test/callers.db.test.mjs",
    file: "services/live/lib/app.mjs",
    find: "    const t = mintCounter.take(addr);\n    if (!t.ok) {",
    replace: "    const t = mintCounter.take(addr);\n    if (false) {",
    needs: "TEST_DATABASE_URL",
    why: "new caller ids are capped per address, which bounds the rows a cookie-less script can create",
  },
  {
    id: "live-talkers-skip-first-call",
    test: "services/live/test/callers.db.test.mjs",
    file: "services/live/lib/app.mjs",
    find: "    if (spoken.has(ref) || (await store.hasSpoken(ref))) {",
    replace: "    if (false) {",
    needs: "TEST_DATABASE_URL",
    why: "callers already talking are never held back by their address's caps or holds",
  },
  {
    id: "live-report-distinct-addresses",
    test: "services/live/test/callers.db.test.mjs",
    file: "services/live/lib/store.mjs",
    find: "        `SELECT count(DISTINCT COALESCE(addr_ref, client_ref))::int AS n FROM live_reports WHERE message_id = $1`,",
    replace: "        `SELECT count(*)::int AS n FROM live_reports WHERE message_id = $1`,",
    needs: "TEST_DATABASE_URL",
    why: "one person with many cookies is one reporter: three cookies must not hide a call",
  },
  {
    id: "live-legacy-adopt",
    test: "services/live/test/callers.db.test.mjs",
    file: "services/live/lib/app.mjs",
    find: "    const adopted = await store.adoptLegacy(addr, ref, LEGACY_ADOPT_MS);",
    replace: "    const adopted = null;",
    needs: "TEST_DATABASE_URL",
    why: "callers from before per-browser ids keep their names: the first browser from their address takes them over",
  },
  {
    id: "live-client-rate-only-for-rate",
    test: "src/services/live/__tests__/client.test.ts",
    file: "src/services/live/client.ts",
    find: "  if (status === 429 && json.error === \"rate\" && typeof json.retryAfter === \"number\") {\n    return { ok: false, reason: \"rate\", message: `Hold the line.",
    replace: "  if (status === 429 && typeof json.retryAfter === \"number\") {\n    return { ok: false, reason: \"rate\", message: `Hold the line.",
    why: "a held or busy network's refusal must say so, not tell the caller to wait an hour's worth of seconds",
  },
  {
    id: "live-numbered-name-when-full",
    test: "services/live/test/callers.db.test.mjs",
    file: "services/live/lib/app.mjs",
    find: "      const name = i < 4 ? randomName() : numberedCallerName(randomInt, randomName, (n) => moderate.name(n).ok);",
    replace: "      const name = randomName();",
    needs: "TEST_DATABASE_URL",
    why: "a night that holds every plain name still names new callers, and admin clear-name still works",
  },
  {
    id: "live-numbered-name-filtered",
    test: "services/live/test/names.test.mjs",
    file: "services/live/lib/names.mjs",
    find: "    if (name.length <= MAX_NAME_CHARS && accept(name)) return name;",
    replace: "    if (name.length <= MAX_NAME_CHARS) return name;",
    why: "a numbered name the filter would refuse is never shown on the lines",
  },
  {
    id: "live-loadtest-gate",
    test: "services/live/test/messages.db.test.mjs",
    file: "services/live/lib/app.mjs",
    find: "    return loadTest && LOOPBACK.has(peer) && typeof key === \"string\" ? key.slice(0, 64) : null;",
    replace: "    return LOOPBACK.has(peer) && typeof key === \"string\" ? key.slice(0, 64) : null;",
    needs: "TEST_DATABASE_URL",
  ...
  },
  {
    ...
    test: "services/live/test/messages.db.test.mjs",
    file: "services/live/lib/app.mjs",
    find: "    hub.broadcast(\"message\", msg, msg.id);",
    replace: "    void msg;",
    needs: "TEST_DATABASE_URL",
  ...
  {
    id: "welcome-offers-live",
    test: "src/app/__tests__/welcome-live.test.tsx",
    file: "src/app/page.tsx",
    find: '            onClick={() => handleEnter("/live")}',
    replace: "            onClick={() => handleEnter()}",
    why: "a first visit never learned there was a live station",
  },
  {
    id: "live-stop-leaves",
    test: "src/audio/__tests__/live-station.test.ts",
    file: "src/audio/live-controller.ts",
    find: "      if (!s.currentEpisode && prev.currentEpisode) {",
    replace: "      if (!s.currentEpisode && prev.currentEpisode && false) {",
    why: "■ Stop while live held the station with an empty player instead of leaving",
  },
  {
    id: "live-seek-refused",
    test: "src/audio/__tests__/live-station.test.ts",
    file: "src/hooks/useAudioPlayer.ts",
    find: "      if (liveLocked()) {\n        toast.info(LIVE_LOCKED_MESSAGE);\n        return;\n      }\n      const audio = getAudio();",
    replace: "      const audio = getAudio();",
    why: "a seek while live was silently undone by the next drift check",
  },
  {
    id: "live-locked-not-when-held",
    test: "src/audio/__tests__/live-station.test.ts",
    file: "src/stores/live-store.ts",
    find: "  return tuned && !paused;",
    replace: "  return tuned;",
    why: "held paused, the listener's own seek and speed are theirs again",
  },
  {
    id: "live-plays-at-1x",
    test: "src/audio/__tests__/live-station.test.ts",
    file: "src/hooks/useAudioPlayer.ts",
    find: "        currentLiveStart()?.fileHash === episode.fileHash ? 1 : usePlayerStore.getState().playbackRate;",
    replace: "        usePlayerStore.getState().playbackRate;",
    why: "at 1.5x the station drifted and the resync pulled it back every ten seconds",
  },
  {
    id: "live-rate-button-refused",
    test: "src/components/player/__tests__/live-rate-lock.test.tsx",
    file: "src/components/player/PlaybackControls.tsx",
    find: "  const cycleRate = () => {\n    if (liveLocked()) {",
    replace: "  const cycleRate = () => {\n    if (false) {",
    why: "the speed button changed a rate the station then fought",
  },
  {
    id: "live-studio-grid-shrinks",
    test: "src/components/live/__tests__/live-station.test.tsx",
    file: "src/components/live/LiveStation.tsx",
    find: '"grid-cols-[minmax(0,1fr)] md:grid-cols-[minmax(0,1fr)_minmax(300px,380px)]"',
    replace: '"md:grid-cols-[minmax(0,1fr)_minmax(300px,380px)]"',
    why: "at 390 wide the studio was 555 px and the clock and live count were off screen",
  },
  {
    id: "live-chat-fills-window",
    test: "src/components/live/__tests__/live-station.test.tsx",
    file: "src/components/live/LiveStation.tsx",
    find: '<div className="h-full min-h-0 flex flex-col" data-testid="phone-lines-body">',
    replace: '<div className="flex-1 min-h-0 flex flex-col" data-testid="phone-lines-body">',
    why: "the call-in box started below the fold on desktop, the list parked on the oldest call",
  },
  {
    id: "live-count-one-wording",
    test: "src/components/live/__tests__/live-chat.test.tsx",
    file: "src/components/live/LiveChat.tsx",
    find: "      {live} tuned in live",
    replace: "      {live} listening live",
    why: "the same number was 'tuned in live' in the studio and 'listening live' beside it",
  },
