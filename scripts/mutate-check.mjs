...
export const MUTATIONS = [
  ...
  },
  {
    ...
    file: "services/mirror/lib/range.mjs",
    find: "const start = Number(m[1]);",
    replace: "const start = Number(m[1]) + 1;",
    why: "an off-by-one range start shifts every seek by a byte: garbage frames and a wrong Content-Range",
  },
  {
    id: "mirror-dht-ipv4",
    test: "services/mirror/test/bootstrap.test.mjs",
    file: "services/mirror/lib/bootstrap.mjs",
    find: "const { address } = await lookup(host, { family: 4 });",
    replace: "const { address } = await lookup(host);",
  ...
  },
  {
    ...
    find: "      .sort((a, b) => a.lastAccess - b.lastAccess);",
    replace: "      .sort((a, b) => b.lastAccess - a.lastAccess);",
    why: "evicting most-recent first throws out the show someone is listening to",
  },
  {
    id: "gateway-pins-survive",
    test: "services/mirror/test/gateway.test.mjs",
    file: "services/mirror/lib/cache.mjs",
    find: "      .filter((e) => !e.pinned && !this.busy.has(e.infohash))",
    replace: "      .filter((e) => !this.busy.has(e.infohash))",
    why: "pinned files are the warm cache — the only shows guaranteed to play while archive.org is down",
  },
  {
    id: "warm-budget",
  ...
  },
  {
    ...
    file: "services/mirror/lib/gateway.mjs",
    find: "      if (!fh || !(await cache.isComplete(ih))) continue;",
    replace: "      if (!fh) continue;",
    why: "a partial download in the manifest sends a listener into a 15 s wait for a 503",
  },
  {
