...
export function Profile({ user, onClose, className }: ProfileProps) {
  if (!user || !user.name) {
    return (
      <div
        className={cn(
      ...
      >
        ...
        <div className="flex items-center justify-between px-3 py-2 border-b border-bevel-dark/20 glass-divider">
          <span className="text-[11px] md:text-[9px] text-bevel-dark/70">User Profile</span>
          <button
            onClick={onClose}
  ...
  const episodes = useLiveQuery(
    () => db.episodes.where("guestName").equals(user.name).sortBy("airDate"),
    [user.name],
  );
    ...
    return (
      ...
      >
        ...
        <div className="p-3 pb-[calc(0.75rem+var(--safe-bottom))] md:pb-3 flex flex-col gap-3 max-h-[80vh] md:max-h-none overflow-auto overscroll-contain md:flex-1">
          ...
          {user.email && (
            <div className="text-[12px] md:text-[10px] text-bevel-dark/70">
              {user.email}
            </div>
          )}
          <div className="text-[14px] md:text-[12px] text-desktop-gray text-center py-8">
