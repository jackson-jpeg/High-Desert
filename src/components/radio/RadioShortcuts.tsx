  ...
  return (
    <div className={cn("relative", className)}>
      <button
        ...
        className="text-hd-9 text-bevel-dark/85 hover:text-desert-amber/85 cursor-pointer transition-colors-fast px-1"
      ...
      {open && (
        <div className="absolute bottom-full right-0 mb-1 w98-raised-dark bg-raised-surface p-3 z-50 w-[220px] shadow-lg">
          <div className="text-hd-9 text-desert-amber/85 font-bold mb-2 uppercase tracking-wider">
            Radio Shortcuts
          </div>
          <div className="flex flex-col gap-0.5">
            {RADIO_SHORTCUTS.map(({ keys, action }) => (
              <div key={keys} className="flex items-center justify-between gap-2">
                <span className="text-hd-9 text-desktop-gray/85">{action}</span>
                <span className="text-hd-8 text-desert-amber/85 tabular-nums flex-shrink-0">{keys}</span>
              </div>
            ))}
          </div>
        </div>
