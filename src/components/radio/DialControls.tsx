...
export function DialControls({
  ...
  const handleFrequencyInput = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Handle AM frequency range (530-1700 kHz)
    const num = parseFloat(raw);
    let clamped: number;
    let band: 'AM';
    
    // Strict AM band validation (530-1700 kHz)
    if (num >= 530 && num <= 1700) {
      clamped = Math.max(530, Math.min(1700, num));
      band = 'AM';
    } else if (num < 530) {
      // Values below AM range clamp to minimum
      clamped = 530;
      band = 'AM';
    } else {
      // Values above AM range clamp to maximum
      clamped = 1700;
      band = 'AM';
    }
