    ...
    return (
      <div className="w98-inset-dark bg-inset-well p-4 animate-slide-up">
        <div className="flex flex-col items-center gap-2 py-2">
          <span className="text-hd-14 text-bevel-dark/30 select-none">{"\u266B"}</span>
          <div className="text-hd-13 md:text-hd-11 text-bevel-dark text-center">
            Queue is empty
          </div>
          <div className="text-hd-13 md:text-hd-10 text-bevel-dark/50 text-center leading-relaxed">
    ...
    <div className="w98-inset-dark bg-inset-well animate-slide-up">
      <div className="flex items-center justify-between px-3 py-1.5 border-b border-bevel-dark/15">
        <div className="flex items-center gap-2">
          <span className="text-hd-13 md:text-hd-11 text-bevel-dark/70">
            {queue.length} track{queue.length !== 1 ? "s" : ""}
          </span>
          {remainingMinutes > 0 && (
            <span className="text-hd-11 md:text-hd-10 text-bevel-dark/40 tabular-nums">
        ...
        <div className="flex items-center gap-1">
          {isAdmin && !savingPlaylist && (
            <button
              ...
              className="text-hd-12 md:text-hd-10 text-bevel-dark/50 hover:text-desert-amber active:text-desert-amber cursor-pointer transition-colors-fast min-h-[44px] md:min-h-0 flex items-center px-2"
          ...
          <button
            onClick={clearQueue}
            className="text-hd-12 md:text-hd-10 text-bevel-dark/50 hover:text-red-400 active:text-red-400 cursor-pointer transition-colors-fast min-h-[44px] md:min-h-0 flex items-center px-2"
          >
            Clear
          </button>
        </div>
      ...
      {savingPlaylist && (
        <div className="flex items-center gap-1.5 px-3 py-1.5 border-b border-bevel-dark/15 bg-midnight/50">
          ...
          <button
            ...
            className="text-hd-12 md:text-hd-10 text-static-green disabled:text-bevel-dark/30 cursor-pointer"
          >
            Save
          </button>
          <button
            onClick={() => setSavingPlaylist(false)}
            className="text-hd-12 md:text-hd-10 text-bevel-dark/50 cursor-pointer"
          >
            Cancel
          </button>
        </div>
        ...
        {queue.map((ep, i) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-hd-11 md:text-hd-10 text-bevel-dark/20 cursor-grab active:cursor-grabbing flex-shrink-0 opacity-100 md:opacity-0 md:group-hover:opacity-60 transition-opacity">
                {"\u2261"}
              </span>
              ...
              <div className="flex flex-col gap-0.5 md:hidden flex-shrink-0">
                <button
                  ...
                  disabled={i === 0}
                  className="text-hd-10 text-bevel-dark/40 active:text-desktop-gray disabled:opacity-20 cursor-pointer px-1"
                  aria-label="Move up"
                >
                  {"\u25B2"}
                </button>
              ...
              {isCurrent && (
                <span className="text-hd-12 md:text-hd-10 text-static-green/70 flex-shrink-0">
                  {"\u25B6"}
                </span>
              )}
              <div className="flex-1 min-w-0">
                ...
                {(ep.guestName || ep.airDate) && (
                  <div className="text-hd-12 md:text-hd-10 text-bevel-dark/60 truncate" title={[ep.guestName, formatAirDate(ep.airDate)].filter(Boolean).join(" \u00B7 ")}>
              ...
              <button
                ...
                className="text-hd-12 md:text-hd-10 text-bevel-dark/50 hover:text-red-400 active:text-red-400 cursor-pointer flex-shrink-0 px-1 min-w-[44px] min-h-[44px] md:min-w-0 md:min-h-0 flex items-center justify-center opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity"
                title="Remove"
              >
                {"\u2715"}
              </button>
